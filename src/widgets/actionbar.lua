...
table.insert(glyphListEntries, {
  ...
  toggle = true,
  value = function() return config.config.controllerGlyphs == '' end
})
for _, layout in ipairs(glyphsList) do
  table.insert(glyphListEntries, {
    ...
    toggle = true,
    value = function() return config.config.controllerGlyphs == layout end
  })
end

local schemesEntries = {}
for _, theme in ipairs(colors.getSchemes()) do
  table.insert(schemesEntries, {
    theme.name,
    click = function()
...
ActionBarWidget.barItems = {
  { 'File',
    {
      ...
      },
      {
        ...
        disabled = function() return not chart.chartDir end
      },
      {
        'Reload',
        click = function()
    ...
    {
      {
        ...
        disabled = function() return #chart.history <= 1 end,
        bind = keybinds.binds.undo
      },
      {
        'Redo',
        click = function()
        ...
        disabled = function() return #chart.future == 0 end,
        bind = keybinds.binds.redo
      },
      {},
      {
        'Cut',
      ...
      },
      {
        ...
        disabled = function() return not edit.hasSomethingToPaste() end,
  ...
  { 'Options', {
    ...
    {},
    {
      ...
      toggle = true,
      value = function() return not (not catjam or catjam.delete) end
    },
    {
